================================================================================
    MFC 程式庫 : C_MFC_Sample 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 C_MFC_Sample 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 C_MFC_Sample 應用程式的檔案。

C_MFC_Sample.vcxproj
    這是使用應用程式精靈所產生之 VC++ 專案的主要專案檔。 
    它包含產生檔案時的 Visual C++ 版本資訊，以及使用應用程式精靈產生檔案的
    過程中，選取的平台、組態和專案功能的相關資訊。

C_MFC_Sample.vcxproj.filters
...
C_MFC_Sample.h
    ...
    並宣告 CC_MFC_SampleApp 應用程式類別。

C_MFC_Sample.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CC_MFC_SampleApp。
...
res\C_MFC_Sample.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    C_MFC_Sample.rc 內。

res\C_MFC_Sample.rc2
...
C_MFC_SampleDlg.h、C_MFC_SampleDlg.cpp - 對話方塊
    這些檔案包含您的 CC_MFC_SampleDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 C_MFC_Sample.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 C_MFC_Sample.pch 的先行編譯標頭 (PCH) 檔，
